...
{#snippet StepsOne()}
	...
	>
		...
		<div
			class="h-[clamp(0.4rem,0.5vw,0.75rem)] w-[clamp(8rem,18vw,22rem)] rounded-full bg-linear-to-r from-[#54CD4C] via-[#3F9A39] to-[#2A6726] mb-[clamp(1.5rem,4vh,3rem)]"
		></div>
...
{#snippet StepsTwo()}
	...
	>
		<div class="flex flex-row items-center justify-center gap-[clamp(0.75rem,2vw,2rem)]">
			...
			/>
			<h1
				class="text-[#54CD4C] font-semibold tracking-tighter leading-none"
				style="font-size: clamp(3rem, 8vw, 9rem);"
			>
				TeaLinuxOS
			</h1>
		</div>
	...
	<Dialog.Root>
		<Dialog.Trigger class="mt-2">
			<Button
				...
				style="font-size: clamp(0.8rem, 1.1vw, 1rem); padding: clamp(0.4rem,1vh,0.6rem) clamp(1.25rem,2.5vw,2rem);"
			>
			...
			<section class="flex flex-col gap-y-4 mt-2">
				{#each CHANGELOG_LISTS as item (item)}
					<div class="flex flex-row gap-x-4">
						<div
							class="size-6 rounded-full bg-[#54CD4C]/10 flex items-center justify-center shrink-0"
						>
